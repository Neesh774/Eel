...
public class Game extends JPanel {
    ...
    private double inversionTime = 0;
    ...
    private void update() {
        snake.move();
        inversionTime = (Math.random() * 3);
        if(inversionTime > 2.9){
            inverted = true;
        }
        if(inverted){
            inversionTime --;
        ...
        if (cherry != null && snake.getHead().intersects(cherry, 40)) {
            double backwardsChance = (Math.random() * 3000);
            if(backwardsChance > 2500){
                snake.invert();
            }
        }
    ...
    private void render(Graphics g) {
        ...
        if (status == GameStatus.NOT_STARTED) {
          drawCenteredString(g2d, "EEL", FONT_XL, 200);
        ...
        g2d.drawString("BEST: " + String.format ("%04d", best), 640, 30);
        ...
        if(inverted && status == GameStatus.RUNNING){
            g2d.drawString("Inverted!", 400, 20);
        }
        ...
        g2d.drawRect(20, 40, WIDTH-20, HEIGHT-20);
    }
    ...
    private class KeyListener extends KeyAdapter {
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if (status == GameStatus.RUNNING) {
                switch(key) {
                    case KeyEvent.VK_LEFT: if(Game.inverted) snake.turn(Direction.RIGHT); else snake.turn(Direction.LEFT); break;
                    case KeyEvent.VK_RIGHT: if(Game.inverted) snake.turn(Direction.LEFT); else snake.turn(Direction.RIGHT); break;
                    case KeyEvent.VK_UP: if(Game.inverted) snake.turn(Direction.DOWN); else snake.turn(Direction.UP); break;
                    case KeyEvent.VK_DOWN: if(Game.inverted) snake.turn(Direction.UP); else snake.turn(Direction.DOWN); break;
                }
            }
    ...
    private class GameLoop extends java.util.TimerTask {
        public void run() {
            update();
            int lagChance = (int)(Math.random() * 2000);
            if(lag == 0){
                if(lagChance >= 1800){
                    lag = (lagChance-300)/600;
                }
                else repaint();
            }
            else{
